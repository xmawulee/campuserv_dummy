...
- **Portfolio & Photo Data Synchronization:** When providers upload images via `POST /users/{id}/portfolio`, files are stored on disk in `/users/files/**` via `FileStorageService`. The backend updates both `User.portfolio` (list of strings) and `ProviderProfile.portfolioUrls` (comma-separated string), eliminating data fragmentation between general user profiles and seller listing carousels. Note that a listing by a provider does not, by default, contain the profile picture of that provider; the provider must manually upload or take a picture of the pictures they want to display on their listings.
- **Multi-Listing and Listing Editing Capabilities:** Providers can edit their already posted listings or create new service listings altogether. This allows a single provider to manage and display multiple active service listings under their account.
...
- **Feed Discovery Endpoint (`GET /api/users/providers`):**
  ...
  - *Response Model (`UserProfileResponse`):* Returns hero photo (first portfolio image only, not defaulting to the profile picture), verified checkmark status, star rating (`rating`), total completed jobs, live view count (`viewCount`), and an `isSaved` boolean (calculated by checking if a record exists in `saved_listings` for the calling user's `X-User-Id`).
